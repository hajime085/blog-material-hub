...
PROMO_WORDS = ["送料無料", "あす楽", "ポイント", "最安", "在庫処分", "セール", "SALE",
               ...
               # 括弧の先頭に置かれる期間・値引きの煽り。
               # 【最大 半額 8/30(日)23:59マデ】が剥がせず商品名に残っていた。
               "半額", "タイムセール", "マデ", "まで", "時間限定", "本日", "今だけ",
               "特価", "均一", "OFF", "オフ", "%"]
...
def clean_title(raw):
    ...
    LEAD = (r"(\d+\s*[%％]\s*(?:OFF|オフ|off)[!！]?|店内最安値|最安値挑戦?中?|最安値|"
            ...
            r"売り切れ次第終了|在庫限り|残りわずか|"
            # 「週末限定セール 南まぐろ」のように、括弧も記号も無い頭の煽り
            r"[^ 　]{0,4}限定セール|週末限定|本日限定|今だけ|\d+\s*時間限定)")
    # 【8月26日（水）00:00～23:59】のように、中身が日時だけの括弧。
    # 販促の語が1つも無いので、これまで剥がせず商品名の頭に残っていた。
    DATE_ONLY = re.compile(r"^[\s\d年月日（）()月火水木金土日曜:：～〜\-–—/.、,]+$")
    ...
    for _ in range(10):
        before = t
        # ＼…／ の中身。日付の「8/29」で切れないよう、閉じ記号は開き記号に
        # 合わせる。以前は [^／/] だったので ＼24時間タイムセール☆8/29 10:00～／
        # が「8/」で切れ、「29 10:00～／」が商品名の頭に残っていた。
        t = re.sub(r"＼[^／]{0,44}／", " ", t)
        t = re.sub(r"\\[^/]{0,44}/", " ", t)
        t = re.sub(r"[★☆◆◇■□●○▼▲！!]+", " ", t)              # 装飾記号
        t = re.sub(r"\s+", " ", t).strip()
        t = re.sub(r"^\d\.\d{1,2}\s*(?=[^\d\s])", "", t)     # 先頭に紛れた評価値
        ...
        # 閉じ記号は開き記号に合わせる。以前はどの閉じ記号でも止まったので、
        # 【最大 半額 8/30(日)23:59マデ】が「(日)」の ) で切れて剥がせなかった。
        m = re.match(r"\s*(?:【([^】]{1,44})】|《([^》]{1,44})》|『([^』]{1,44})』"
                     r"|\[([^\]]{1,44})\]|（([^）]{1,44})）|\(([^)]{1,44})\))", t)
        if m:
            inner = next(g for g in m.groups() if g is not None)
            if (any(w in inner for w in PROMO_WORDS)
                    or PRICE_PITCH.search(inner) or UNIT_RE.search(inner)
                    or (DATE_ONLY.match(inner) and re.search(r"\d", inner))):
                t = t[m.end():]
...
def fetch_event(cfg, app_id, access_key, aff_id, site_url,
    ...
    # イベントの最中は多めに、それ以外の日も少しは拾う。
    #
    # 以前はイベントの外ではまったく回さなかった。
    # その結果、トップの「まもなく始まる特価」が月のほとんどの日で
    # 2〜3件しかない状態になっていた。いちばん上に置いている枠が
    # 空に近いのは、読み物として弱い。
    #
    # 楽天には「◯月◯日10時〜」のタイムセールが常時ある。
    # イベントでなくても拾えるものはあるので、毎日少しずつ足す。
    ev = None if force else event_window(r.get("eventLeadHours", 12))
    everyday = not force and ev is None
    if ev:
        print("▼ %s に合わせて回します（%s 〜 %s）"
              % (ev.get("name", "イベント"), ev.get("start"), ev.get("end")))
    elif everyday:
        print("▼ イベントの期間外です。開始前の特価を少しだけ足します。")
    ...
    if everyday:
        # 普段の日はイベントほど数が出ないし、キャプションも追いつかない。
        # 枠が寂しくならない程度に留める。
        target = min(target, r.get("everydayMaxNewPerRun", 4))
